...
export type DocStatus = "Available" | "Admin" | "Partial" | "Coming Soon";
...
const docSections: DocSection[] = [
  ...
  },
  {
    ...
    technicalDetails: [
      ...
      "Project-scoped chat uses context-aware routing before RAG so direct chat, task extraction, web search, artifact generation, and Vectorize retrieval use separate backend paths.",
  ...
  },
  {
    ...
    technicalDetails: [
      ...
      "The useWorkspaceEventSource hook consumes mutation events with EventSource, persists the latest native EventSource event id in sessionStorage, deduplicates event ids with an in-memory set, and invalidates pmoWorkspaceQueryKey, my-teams, scoped-projects, or scoped-chats based on invalidates_json.",
  ...
  },
  {
    ...
    details: [
      ...
      "Tasks stay local until the user clicks Sync to Asana unless auto-sync is enabled in Profile > Asana; optimistic UI marks the sync Pending while the queue consumer works.",
    ...
    technicalDetails: [
      ...
      "VertexAI task sync writes the local D1 task first, queues an ASANA_SYNC_QUEUE job, creates the remote Asana task in the consumer, then stores asana_task_gid, asana_synced_at, outbound_status, and sync_status back on workspace_actions.",
      "ASANA_OUTBOUND_QUEUE isolates direct Asana outbound HTTP calls from foreground request handling and records Pending, Sent, and Failed delivery state on workspace_actions.",
  ...
  },
  {
    ...
    details: [
      "The standalone Cloudflare Worker schedule wakes hourly for scheduled_tasks and runs a Monday 0 12 * * 1 agentic briefing cron.",
    ...
    technicalDetails: [
      "wrangler.jsonc defines cron triggers for 0 * * * * scheduled task dispatch and 0 12 * * 1 weekly agentic briefings.",
      ...
      "runWeeklyAgenticBriefings summarizes recent workspace actions, chat activity, ideas, and web context into the briefings table with a source-data hash.",
  ...
  },
  {
    ...
    technicalDetails: [
      ...
      "When WORKSPACE_INTELLIGENCE_QUEUE is available, new ideas enqueue a background multi-agent evaluation that writes normalized category, severity, and mitigation findings to project_risks.",
  ...
  },
  {
    ...
    technicalDetails: [
      ...
      "Task rows expose outbound_status and sync_status so the UI can distinguish Pending, Sent, Failed, and NotQueued Asana sync states.",
  ...
  },
  {
    ...
    technicalDetails: [
      ...
      "src/lib/auth.ts creates Better Auth from the current request environment and keeps tanstackStartCookies as the final plugin so Cloudflare edge requests do not share stale auth or cookie state.",
  ...
  },
  {
    ...
    technicalDetails: [
      ...
      "src/db/index.ts and src/dbMiddleware.ts provide request-scoped D1 and Drizzle context so server code does not depend on a module-level database singleton.",
      ...
      "Cloudflare Worker deployment reads DB, ARTIFACTS_BUCKET, VECTORIZE, Queues, AI, CHAT_SYNC, and cron triggers from wrangler.jsonc, while Sites declares logical bindings in .openai/hosting.json.",
  ...
  },
  {
    ...
    status: "Partial",
    summary:
      "Context-aware routing decides whether prompts need Vectorize, web search, task extraction, direct chat, or artifact generation while document ingestion keeps project artifacts indexed.",
    howTo: [
    ...
    details: [
      ...
      "The router recognizes RAG_SEARCH, WEB_SEARCH, DIRECT_CHAT, ENTITY_EXTRACTION, TASK_EXTRACTION, and ARTIFACT_GENERATION as strict intent categories.",
    ...
    technicalDetails: [
      "Scoped RAG requires DB, ARTIFACTS_BUCKET, VECTORIZE, DOCUMENT_INGESTION_QUEUE, WORKSPACE_INTELLIGENCE_QUEUE, and AI bindings.",
      ...
      "document-ingestion-worker handles queued jobs and owns expensive ingestion work: document text extraction, semantic chunking, vector_tenant_id resolution, embedTexts execution, D1 document_chunks or document_chunks_v2 inserts, and Vectorize upserts.",
      ...
      "RAG_SEARCH embeds the prompt, queries Vectorize with vector_tenant_id plus role-sensitive confidentiality metadata filters, revalidates matching chunks against D1 scope and role rules, and streams a cited answer.",
      "WEB_SEARCH calls fetchConsolidatedWebSearch, loads scoped D1-backed historical chunks through tenant-filtered Vectorize matches, wraps both context sections as untrusted_context, and streams an answer from both sections.",
      "DIRECT_CHAT, ENTITY_EXTRACTION, TASK_EXTRACTION, and ARTIFACT_GENERATION bypass embeddings, Vectorize, and web search, then stream from the primary generation model with the same priority workspace context and role directive.",
      ...
      "The scoped RAG SSE contract emits trace, citations, thinking, token, entities, done, and stream-error events. The client closes the EventSource on done or stream-error, while transport failures use the native onerror path and server-side stream cancellation closes the active Workers AI reader.",
      "Intent routing uses @cf/meta/llama-3-8b-instruct to choose RAG_SEARCH, WEB_SEARCH, DIRECT_CHAT, ENTITY_EXTRACTION, TASK_EXTRACTION, or ARTIFACT_GENERATION.",
  ...
  },
  {
    ...
    technicalDetails: [
      ...
      "Document ingestion now uses an asynchronous R2 plus Queue handoff and tenant-scoped Vectorize metadata, but the UI still needs richer status polling, retry visibility, and expanded parser coverage.",
...
const technicalArticles: DocArticle[] = [
    ...
    blocks: [
      {
        ...
        items: [
          ...
          "Generated scoped RAG artifacts can pass sensitivityLabel Confidential or restricted true to store matching R2 metadata before queue ingestion.",
          "RAG_SEARCH embeds the prompt, queries Vectorize with vector_tenant_id and role-sensitive confidentiality metadata filters, revalidates chunks against D1 scope and role rules, and streams a cited answer.",
          "The scoped RAG SSE contract emits trace, citations, thinking, token, entities, done, and stream-error events. The client closes the EventSource on done or stream-error, while transport failures use the native onerror path and server-side cancellation closes the active Workers AI reader.",
          "Citation metadata includes vector id, document name, R2 key, and match score when available.",
        ],
      },
    ],
  },
  {
    ...
    status: "Partial",
    summary:
      "Semantic search stores embedded document chunks in Vectorize and retrieves them with compressed tenant and role-sensitive confidentiality metadata filters.",
    blocks: [
      {
        title: "Implementation Details",
        variant: "technical",
        items: [
          "Document ingestion creates semantic chunks, resolves a vector_tenant_id, embeds chunk text, writes chunk records to D1, and upserts corresponding vectors into Vectorize.",
          "Vector queries are filtered by vector_tenant_id so project chat retrieval stays scoped to the active team project without long metadata values.",
          ...
          "D1 chunk lookup revalidates team, project, and confidentiality fields after Vectorize matching before context reaches the model.",
          "D1 remains the source of chunk metadata and readable content, while Vectorize provides similarity search and match scores.",
          "Prompt embedding happens for RAG_SEARCH and WEB_SEARCH intents; direct chat, task extraction, entity extraction, and artifact generation bypass embeddings and Vectorize.",
    ...
    blocks: [
      {
        ...
        items: [
          "Project-scoped chat uses context-aware routing before RAG so direct chat, task extraction, web search, artifact generation, and Vectorize retrieval use separate backend paths.",
          "Intent routing chooses RAG_SEARCH, WEB_SEARCH, DIRECT_CHAT, ENTITY_EXTRACTION, TASK_EXTRACTION, or ARTIFACT_GENERATION.",
          "WEB_SEARCH calls fetchConsolidatedWebSearch, loads scoped D1-backed historical chunks through tenant-filtered Vectorize matches, wraps context as untrusted_context, and streams an answer from both context sections.",
          "DIRECT_CHAT, ENTITY_EXTRACTION, TASK_EXTRACTION, and ARTIFACT_GENERATION bypass embeddings, Vectorize, and web search, then stream from the primary generation model with the D1-backed priority workspace context header.",
          "Reasoning profiles map to scoped context budgets, max completion tokens, optional reasoning_effort, and thinking visibility settings.",
        ],
      },
    ],
    ...
    blocks: [
      {
        ...
        items: [
          "uploadArtifact writes the raw file to ARTIFACTS_BUCKET, inserts an artifacts_registry row with pending status, publishes a DocumentIngestionJob to DOCUMENT_INGESTION_QUEUE, sets HTTP 202 Accepted, and returns queued.",
          "ingestGeneratedArtifact writes raw artifact text to R2 and queues a scoped-rag-generated-artifact job.",
          "document-ingestion-worker handles queued jobs and owns expensive ingestion work: document text extraction, semantic chunking, vector_tenant_id resolution, embedTexts execution, D1 chunk inserts, and Vectorize upserts.",
          "wrangler.jsonc binds DOCUMENT_INGESTION_QUEUE as the producer for the main app, while wrangler.document-ingestion.jsonc declares the document-ingestion-queue consumer with batch size 5, timeout 30, and max retries 3.",
          "The artifact status starts as pending, moves to processing in the Queue consumer, and finishes as completed or failed after background ingestion.",
        ],
      },
    ...
    blocks: [
      {
        ...
        items: [
          ...
          "sendChatMessage persists both the user message and assistant response to chat_messages after permission checks pass.",
          "Project-scoped RAG chat uses /sse/workspace-events with stream=scoped-rag and the browser EventSource API when Team project context, AI, Vectorize, queue, DB, and R2 bindings are configured.",
          "Asana task sync uses optimistic Pending state, queue-backed completion, and rollback on mutation failure.",
          "Scoped RAG tokens are appended to the optimistic assistant message as SSE token events arrive, so Markdown renders incrementally during generation.",
          "Chat updates are published through CHAT_SYNC and consumed by /api/chat-events as server-sent events.",
        ],
      },
    ...
    blocks: [
      {
        ...
        items: [
          ...
          "Cloudflare Worker deployment reads DB, ARTIFACTS_BUCKET, VECTORIZE, Queues, AI, CHAT_SYNC, and cron triggers from wrangler.jsonc, while Sites declares logical bindings in .openai/hosting.json.",
          "Server functions should use src/db/index.ts or src/dbMiddleware.ts for request-scoped D1 access when adding new Worker-backed database flows.",
...
export const docArticles = [...userWorkflowArticles, ...technicalArticles];
